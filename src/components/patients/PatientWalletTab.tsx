...
export default function PatientWalletTab({ patient }: { patient: Patient }) {
  ...
  const loadPackages = async () => {
    ...
    try {
      ...
      const data = await patientPackageService.getPatientPackages(
        patient.id,
        clinicId,
      );
  ...
  const loadTransactions = async () => {
    ...
    try {
      setLoading(true);
      const data = await walletService.getPatientTransactions(
        patient.id,
        clinicId,
      );
  ...
  const handleAddFunds = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({
        title: "Funds Added",
        description: `Successfully added NPR ${depositAmount} to wallet.`,
        color: "success",
      });
      setAmount("");
      setNotes("");
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-surface border border-border-base rounded-[10px] p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-[13px] text-text-muted font-medium uppercase tracking-wide">
              Available Balance
            </h3>
            <p className="text-[28px] font-bold text-text-main leading-tight">
        ...
        <div className="bg-surface border border-border-base rounded-[10px] overflow-hidden">
          ...
          <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            {activePackages.map((pkg) => (
              <div
                key={pkg.id}
                className="border border-border-base rounded-lg p-4 bg-surface-2/30 flex flex-col gap-3"
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-[12px] text-text-muted mt-0.5">
                      Purchased: {pkg.createdAt.toLocaleDateString()}
                    </p>
                    {pkg.expiresAt && (
                      <p
                      ...
                      >
                        {pkg.expiresAt < new Date()
                          ? "Expired on: "
                          : "Expires: "}{" "}
                        {pkg.expiresAt.toLocaleDateString()}
                      </p>
                    )}
                  </div>
                  <span
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {pkg.sessions && pkg.sessions.length > 0
                      ...
                      : // Fallback for legacy packages without explicit tickets
                        Array.from({ length: pkg.totalSessions }).map(
                          (_, i) => (
                            <div
                              key={i}
                              className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold border-2 transition-all ${
                  ...
                  {pkg.sessions && pkg.sessions.length > 0 ? (
                    <div className="mt-4 pt-3 border-t border-border-base/50">
                      <span className="text-[11.5px] font-semibold text-text-muted mb-2 block tracking-wide uppercase">
                        Session Checklist
                      </span>
                      <div className="space-y-1.5 max-h-32 overflow-y-auto pr-1 custom-scrollbar">
                        {pkg.sessions.map((session, idx) => (
                          <div
                            key={idx}
                            className="flex justify-between items-center text-[11px] bg-surface/80 rounded px-2.5 py-1.5 border border-border-base/30"
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-text-main font-semibold">
                                Ticket #{session.sessionNumber}
                              </span>
                              <span
                              ...
                              session.consumedAt && (
                                <span className="text-text-muted flex items-center gap-1.5">
                                  ...
                                  {session.clinicianName && (
                                    <>
                                      <span className="w-1 h-1 rounded-full bg-border-base mx-0.5" />
                                      {session.clinicianName}
                                    </>
                                  )}
                                </span>
      ...
      {showAddModal && (
        ...
        >
          ...
          >
            ...
            <form className="p-4 space-y-4" onSubmit={handleAddFunds}>
              <div>

...
vi.mock('firebase/firestore', async () => {
  ...
  return {
    ...
    Timestamp: class Timestamp {
      ...
      static fromDate(d: any) { return new Timestamp(); }
    },
...
describe('Front Office Patient Journey (End-to-End Flow)', () => {
  ...
  it('Phase 1: Check-in creates consultation bill and payment generates doctor commission', async () => {
    ...
    const billingId = await appointmentBillingService.createBilling(mockBillingData);
    expect(billingId).toBe('mocked_doc_id');

    // 2. Settle the consultation bill
    // Using a spy to verify doctor commission is triggered
    const commissionSpy = vi.spyOn(doctorCommissionService, 'createCommission').mockResolvedValue(undefined);
  ...
  it('Phase 2: Doctor writes prescription and sends to Pharmacy', async () => {
    ...
    const mockPrescription = {
      ...
      items: [
        { medicineId: "med_1", name: "Paracetamol", dosage: "1-0-1", duration: "5 Days", instruction: "After meals" }
      ],
      notes: "Take rest",
    ...
    const rxId = await prescriptionService.createPrescription(mockPrescription as any);
    ...
    const mockPharmacyInvoice = {
      ...
      prescriptionId: rxId,
      items: [
        { medicineId: "med_1", name: "Paracetamol", price: 10, quantity: 10, total: 100 }
      ],
      subTotal: 100,
      discountType: "percent" as const,
      discountValue: 0,
    ...
    vi.spyOn(pharmacyService, 'createMedicinePurchase').mockResolvedValue('mocked_doc_id');

    const pharmacyInvoiceId = await pharmacyService.createMedicinePurchase(mockPharmacyInvoice as any);
  ...
  it('Phase 3: Doctor recommends procedure (Expert assigned)', async () => {
    ...
    const billingId = await appointmentBillingService.createBilling(mockProcedureBill);
    expect(billingId).toBe('mocked_doc_id');

    // Settle procedure bill -> Expert gets commission
    const expertCommissionSpy = vi.spyOn(expertCommissionService, 'createCommissionsFromBilling').mockResolvedValue(undefined);
  ...
  it('Phase 4: Doctor prescribes Pathology tests', async () => {
    const mockPathologyBill = {
      ...
      items: [
        { id: "item_1", testId: "test_1", testName: "CBC", price: 800, isCompleted: false, amount: 800, quantity: 1 }
      ],
      subtotal: 800,
      discountType: "percent" as const,
      discountValue: 0,
    ...
    const pathId = await pathologyBillingService.createBilling(mockPathologyBill as any);

...
export default function TestCommission() {
  ...
  const runEndToEndSimulation = async () => {
    ...
    try {
      ...
      if (!doctorsSnap.empty) {
        ...
        await setDoc(doc(db, "doctors", mockDoctorId), { name: mockDoctorName, defaultCommission: 20 });
      }
      ...
      if (!expertsSnap.empty) {
        ...
        await setDoc(doc(db, "experts", mockExpertId), { name: mockExpertName, defaultCommission: 15 });
      }
      ...
      if (!staffSnap.empty) {
        ...
        await setDoc(doc(db, "users", mockStaffId), { name: mockStaffName, commissionRate: 5 });
      }
      ...
      addLog(`📝 Creating Invoice for NPR ${invoiceTotal} with NPR ${globalDiscount} Global Discount...`);
      const newInvoiceId = await appointmentBillingService.createBilling({
        ...
        staffCommissions: [
          { staffId: mockStaffId, staffName: mockStaffName, commissionAmount: 900, percentage: 5 } // 5% of 18000
        ]
      } as any);
      ...
      const isBlocked = !docComms1.docs.some(d => d.data().billingId === newInvoiceId);

      if (isBlocked) {
        addLog(`🛡️ SUCCESS: Commission engine securely blocked generation on partial payment.`);
      } else {
      ...
      await appointmentBillingService.recordPayment(newInvoiceId, finalAmount - 5000, "cash");

      addLog(`⚙️ Running commission generation engine for fully paid invoice...`);

      const updatedInvoice = await appointmentBillingService.getBillingById(newInvoiceId);
      if (updatedInvoice && (updatedInvoice.paidAmount || 0) >= updatedInvoice.totalAmount) {

        const doctorItems = updatedInvoice.items?.filter(i => i.doctorId === mockDoctorId) || [];
        const expertItems = updatedInvoice.items?.filter(i => i.doctorId === mockExpertId) || [];
      ...
      const expertCommsFinal = await getDocs(collection(db, "expertCommissions"));

      const myDocComm = docCommsFinal.docs.find(d => d.data().billingId === newInvoiceId)?.data();
      const myExpComm = expertCommsFinal.docs.find(d => d.data().billingId === newInvoiceId)?.data();

      if (myDocComm && myDocComm.commissionAmount === 1800) {
        addLog(`✅ SUCCESS: Doctor Commission properly calculated with shared global discount burden (NPR 1800).`);
      } else {
        addLog(`❌ ERROR: Doctor Commission wrong. Expected 1800, got ${myDocComm?.commissionAmount}`);
      }

      if (myExpComm && myExpComm.commissionAmount === 1350) {
        addLog(`✅ SUCCESS: Expert Commission properly calculated with shared global discount burden (NPR 1350).`);
      } else {
        addLog(`❌ ERROR: Expert Commission wrong. Expected 1350, got ${myExpComm?.commissionAmount}`);
      }
  ...
  const wipeFinancialData = async () => {
    if (!window.confirm("Are you ABSOLUTELY sure? This will delete ALL invoices and commissions.")) return;
    ...
    try {
      ...
      const pathBillingsSnap = await getDocs(collection(db, "pathologyBilling"));
      let pathBillCount = 0;
      ...
      addLog(`✅ Reset cached totals and deleted all mock simulation profiles.`);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-2xl font-bold text-primary">🧪 Ultimate Commission Simulation</h1>
          <p className="text-sm text-default-500 mb-4">
            Runs a fully automated UI simulation: Creates clinicians, builds a complex multi-party invoice, applies a global discount, attempts a partial payment, and then forces full payment to prove perfect calculations.
          </p>
          <Button
            color="primary"
        ...
        <div className="pt-8 border-t border-default-200">
          <h1 className="text-2xl font-bold text-danger">⚠️ Danger Zone: Financial Wipe</h1>
          <p className="text-sm text-default-500 mb-4">
            This utility will cleanly wipe all historical test data so you can launch with a clean slate.
            It will delete ALL Invoices and Commissions. It will NOT delete Patient records, but it will sever their "Assigned Doctor/Expert" relations.
          </p>

          <Button
            color="danger"
            onClick={wipeFinancialData}
            disabled={loading}
          >
      ...
      <div className="mt-8 bg-default-50 rounded-xl p-4 border border-default-200 min-h-[300px] font-mono text-sm">
        ...
        {logs.map((log, i) => (
          <div key={i} className={`mb-1 ${log.includes('SUCCESS') || log.includes('✅') ? 'text-success' : log.includes('ERROR') || log.includes('❌') ? 'text-danger' : 'text-default-700'}`}>
            {log}
          </div>
        ))}
        {logs.length === 0 && <span className="text-default-400">No logs yet...</span>}
      </div>
    </div>
  );
}

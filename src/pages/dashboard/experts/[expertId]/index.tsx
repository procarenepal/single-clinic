...
export default function ExpertProfilePage() {
  ...
  const patientNames = patients.reduce(
    (acc, p) => {
      acc[p.id] = p.name;
          ...
          <div className="flex items-center gap-4 shrink-0 w-full lg:w-auto">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center px-2 py-0.5 border rounded-full text-[11px] font-semibold tracking-wide uppercase bg-emerald-500/10 text-emerald-600 border-emerald-500/20">
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
            ...
            <div className="flex flex-col gap-1.5 justify-center md:items-end">
              <span className="flex items-center gap-1.5 text-[13px] text-text-muted hover:text-text-main transition-colors">
                ...
                <CopyButton label="Phone Number" text={expert.phone} />
              </span>
              {expert.email && (
                <span className="flex items-center gap-1.5 text-[13px] text-text-muted hover:text-text-main transition-colors">
                  <IoMailOutline className="text-text-muted/50 w-3.5 h-3.5" />
                  <span>{expert.email}</span>
                  <CopyButton label="Email Address" text={expert.email} />
                </span>
              )}
            </div>
          </div>
                ...
                <div className="bg-surface border border-border-base p-5 rounded-[10px] shadow-sm flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-stat-sm text-amber-600 font-bold mt-1">
                      NPR{" "}
                      {Math.max(
                        (expert.monthlyTarget || 0) - achievementBusiness,
                        0,
                      ).toLocaleString()}
                    </p>
                  </div>

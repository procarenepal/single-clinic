...
async function seedMedicinesDirectly() {
  ...
  const clinics = clinicsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  for (const clinic of clinics) {
    ...
    console.log(`\n--------------------------------------------`);
    console.log(`Cleaning and Seeding for Clinic: ${(clinic as any).name || clinicId}`);
    ...
    if (!usersSnapshot.empty) {
      ...
      console.log(`✓ Using active user: ${userId} (${(usersSnapshot.docs[0].data() as any).name || 'Unnamed'})`);
    } else {
    ...
    // --- SEEDING ---
    try {
      console.log(`⏳ Seeding 50 medicines with complete stock records and audit trails...`);
      await medicineService.seedDefaultMedicines(clinicId, undefined, userId);
      console.log(`✨ Successfully seeded 50 medicines and stock records for clinic ${clinicId}!`);
    } catch (err) {
